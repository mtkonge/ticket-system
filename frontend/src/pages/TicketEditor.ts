...
export class TicketEditor implements Component {
    ...
    public hydrate(update: () => void): void {
        ...
        if (!this.ticket.isFetched) {
            oneTicket({ id: ticketId, token: this.context.session.token }).then(
                (response) => {
                    if (!response.ok) {
        ...
        if (!this.usernames.isFetched && this.ticket.isFetched) {
            const user_ids = [
                this.ticket.data!.assignee,
                this.ticket.data!.creator,
                ...this.ticket.data!.comments.map((comment) => comment.creator),
            ];
            usernames({
                user_ids,
            }).then((response) => {
        ...
        if (
            !this.assignableUsers.isFetched &&
            this.ticket.isFetched &&
            this.usernames.isFetched &&
            this.context.session.role !== "Consumer"
        ) {
            allAssignableUsers({ token: this.context.session!.token }).then(
                (response) => {
        ...
        if (
            ...
            const addCommentForm = domSelectId<HTMLFormElement>(
                this.addCommentFormId,
            );
            const saveEditForm = domSelectId<HTMLFormElement>(this.editFormId);

            domAddEvent<HTMLInputElement, "click">(
                this.editModeId,
                "click",
                async () => {
            ...
            const assignSelect = domSelectId<HTMLInputElement>(
                this.selectAssignId,
            );
            domAddEvent<HTMLInputElement, "click">(
                this.saveAssignId,
